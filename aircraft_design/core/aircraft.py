...
from aircraft_design.components.aerodynamics.wing_geometry import WaypointWingGeometry, TailGeometry
...
class Aircraft(Component):
    ...
    def add_wing(self,
        ...
        wing.update_parameters({
            'span': half_span * 2,  # Convert to full span
    ...
    def add_tail(self,
        ...
        if tail_type == 'vertical':
            ...
            name = "vertical_tail"
        else:  # horizontal tail should have a wing geometry
            tail = WaypointWingGeometry()
            tail.position = position if position else Position(0, 0, 0)
            tail.parameters.update({
                'span': half_span * 2,
                'root_chord': root_chord,
    ...
    def add_engines(self, engine_configs: List[Dict]) -> None:
        ...
        for idx, config in enumerate(engine_configs):
            engine = Engine(f"engine_{idx}")
            engine.geometry.parameters.update({
                'radius': config['radius'],
                'length': config['length']
            })
            
            # Set position for each engine
            for pos_idx, (x, y, z) in enumerate(config['positions']):
                engine_instance = Engine(f"engine_{idx}_pos_{pos_idx}")
                engine_instance.geometry = engine.geometry
                engine_instance.geometry.position = Position(x, y, z)
                self.add_child(engine_instance)  # Add directly to aircraft instead of wing
...
def main():
    ...
    # Add main wing
    aircraft.add_wing(
        half_span=150,  # 300ft total span
        root_chord=65,
        tip_chord=10,
        sweep=37.5,
        dihedral=5,
    ...
    aircraft.add_engines([{
        ...
        'length': 20,
        'positions': [
            (30, 50, 0),   # Adjusted X position to be relative to aircraft
            (30, 100, 0)   # Adjusted X position to be relative to aircraft
        ]
    }])
